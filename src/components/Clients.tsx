...
export default function Clients() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setNewClient({ clientName: '', clientAddress: '', clientContact: '' });
    fetchClients();
  ...
  const filteredClients = clients.filter(c => 
    ...
    c.clientContact.includes(search)
  );
        ...
        {filteredClients.map((client) => (
          <motion.div
            ...
            className={cn(
              ...
              selectedIds.includes(client.id) && "border-indigo-500 bg-indigo-50/30"
            )}
          >
            ...
            <h3 className="text-lg font-bold text-neutral-900 mb-4">{client.clientName}</h3>
            
            <div className="space-y-3">
              <div className="flex items-start gap-3 text-sm text-neutral-500">
                <MapPin className="w-4 h-4 mt-0.5 text-neutral-400" />
                <span>{client.clientAddress || 'No address provided'}</span>
              </div>
              <div className="flex items-center gap-3 text-sm text-neutral-500">
                <Phone className="w-4 h-4 text-neutral-400" />
                <span>{client.clientContact || 'No contact provided'}</span>
              </div>
            </div>
          </motion.div>
        ))}
